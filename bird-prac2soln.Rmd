...
description: |
  Ft Riley 2020 avian survey using covariates for bout-specific estimates.  Minor revision to treat `bout` explicitly as stratum.
author:
  - name: Rexstad 
    url: 
...
birds$Region.Label <- birds$bout  # explicitly make bouts into Strata
...
# plots visit-specific detection functions
  ...
  add_df_covar_line(dsobject, data=data.frame(Region.Label=as.factor(1)), 
                    pdf=TRUE, lty=1, col="blue")
  add_df_covar_line(dsobject, data=data.frame(Region.Label=as.factor(2)), 
                    pdf=TRUE, lty=1, col="green")
  add_df_covar_line(dsobject, data=data.frame(Region.Label=as.factor(3)),
...
nobo.vstrat <- ds(data=nobo,
                transect="point", key="hr", formula=~as.factor(Region.Label),
                truncation = mytruncation, convert.units = birdunits)
nobo.ests <- dht2(ddf=nobo.vstrat, flatfile=nobo,
                  strat_formula = ~as.factor(Region.Label), convert_units = birdunits,
...
hesp.vstrat <- ds(data=hesp,
                transect="point", key="hn", formula=~as.factor(Region.Label),
                truncation = mytruncation, convert.units = birdunits)
...
```{r outests, echo=FALSE, eval=TRUE}
outests <- rbind(dickobs$dht$individuals$D[4,2:6],
                 dickscribe$dht$individuals$D[4,2:6],
                 dickwind$dht$individuals$D[4,2:6],
                 dickstart$dht$individuals$D[4,2:6],
                 dicktemp$dht$individuals$D[4,2:6],
                 dicknothing$dht$individuals$D[4,2:6])
