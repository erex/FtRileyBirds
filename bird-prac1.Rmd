...
each.visit.alone <- function(mydata, spname, mytrunc, myunits) {
  ...
  this.hrc <- ds(data=thisspp,
                  ...
                  truncation = mytrunc, convert.units = myunits)
  v1 <- summarize_ds_models(this.unic, this.hnc, this.hrc)
  bout1 <- this.hrc$dht$individuals$D
  sum.bout.specific <- min(AIC(this.unic, this.hnc, this.hrc)[,2])
# visit2
  ...
  this.hrc <- ds(data=thisspp,
                  ...
                  truncation = mytrunc, convert.units = myunits)
  v2 <- summarize_ds_models(this.unic, this.hnc, this.hrc)
  bout2 <- this.hrc$dht$individuals$D
  sum.bout.specific <- sum.bout.specific + min(AIC(this.unic, this.hnc, this.hrc)[,2])
# visit3  
  ...
  this.hrc <- ds(data=thisspp,
                  transect="point", key="hr", adjustment = "cos",
                  truncation = mytrunc, convert.units = myunits)
  v3 <- summarize_ds_models(this.unic, this.hnc, this.hrc)
  ninemodels <- rbind(v1, v2, v3)
  bout3 <- this.hrc$dht$individuals$D
  sum.bout.specific <- sum.bout.specific + min(AIC(this.unic, this.hnc, this.hrc)[,2])
  ests <- rbind(bout1, bout2, bout3)
  ests$Label <- c("Visit 1", "Visit 2", "Visit 3")
# function returns
#   ests - data frame of bout-specific density estimates from pooled detfn model
#   sum.bout.specific - AIC value summed across bout-specific selected models
#   ninemodels - concatenated model selection table across all bouts
  return(list(ests=ests, sum.bout.specific=sum.bout.specific, ninemodels=ninemodels))
}
```
...
kable(density.table[, 1:6], caption="Density estimates (per square kilometer) from pooled detections for all species", row.names=FALSE, digits=4)
